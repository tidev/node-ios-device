...
function findBinding(): string {
	const _dirname = dirname(import.meta.dirname);

	for (const type of ['Release', 'Debug'] as const) {
		try {
			for (const name of readdirSync(join(_dirname, 'build', type))) {
				if (extRE.test(name)) {
					return resolve(_dirname, 'build', type, name);
				}
			}
		} catch {}
	}
	...
	try {
		for (const target of readdirSync(join(_dirname, 'prebuilds'))) {
			const [platform, arch] = target.split('-');
			if (arch === process.arch && platform === process.platform) {
				for (const binding of readdirSync(join(_dirname, 'prebuilds', target))) {
					if (extRE.test(binding)) {
						return resolve(_dirname, 'prebuilds', target, binding);
					}
				}
			}
		}
